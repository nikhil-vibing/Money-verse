...
const beats = [
  {
    ...
    body: "You wake up in a one-room flat in a city you've never seen. Rent is due in nine days.",
  },
  {
    label: "02",
    title: "Take a quest.",
    body: "Bumi from the corner shop needs help cutting her costs. You see her ledger. You make the call.",
  },
  {
    label: "03",
    title: "Learn the lesson after.",
    body: "The math gets named only after you've done it. You leave knowing one new thing — and feeling it.",
  },
];
      ...
      <div className="mx-auto grid max-w-6xl gap-16 lg:grid-cols-[1fr_1.1fr]">
        <div className="lg:sticky lg:top-32 lg:self-start">
          ...
          <p className="mt-6 max-w-md text-[var(--color-paper-dim)]">
            Each district teaches a domain — budgeting, banking, taxes, investing, protection, work,
            retirement. You unlock the next by showing you can live in this one.
          </p>
            ...
            {beats.map((b, i) => (
              <li
                ...
                className={`flex items-start gap-4 rounded-xl border px-4 py-4 transition-all duration-500 ${
                  i === activeBeat
                    ? "border-[var(--color-crimson)]/60 bg-[var(--color-crimson)]/[0.06]"
...
function MockScene({ activeBeat }: { activeBeat: number }) {
  return (
    <div className="relative">
      <div
        aria-hidden="true"
        className="absolute -inset-6 rounded-[28px] bg-gradient-to-br from-[var(--color-crimson)]/20 via-transparent to-[var(--color-amber)]/10 blur-2xl"
      />
...
function SkyGradient({ activeBeat }: { activeBeat: number }) {
  const skies = [
    "linear-gradient(180deg, #1a0f2e 0%, #2a1845 40%, #4a2160 100%)",
    "linear-gradient(180deg, #2a1845 0%, #6b2e4a 40%, #c25a3e 100%)",
    "linear-gradient(180deg, #1a0f2e 0%, #4a2160 30%, #f5b43a 90%, #ffd87a 100%)",
  ];
  return (
    <div
      aria-hidden="true"
  ...
  return (
    <svg viewBox="0 0 320 60" className="h-12 w-full opacity-60" aria-hidden="true">
      {Array.from({ length: 24 }).map((_, i) => {
        ...
        const r = (i % 3) * 0.4 + 0.6;
        const key = `star-${x}-${y}`;
        return <circle key={key} cx={x} cy={y} r={r} fill="#f6f3eb" opacity={0.6} />;
      })}
    </svg>
  );
}
...
function Cityscape() {
  return (
    <svg viewBox="0 0 320 180" className="w-full" aria-hidden="true">
      ...
      <rect x="44" y="90" width="36" height="90" fill="#11111a" />
      <rect x="50" y="98" width="6" height="6" fill="#f5b43a" />
      <rect x="62" y="98" width="6" height="6" fill="#f5b43a" />
      <rect x="50" y="112" width="6" height="6" fill="#f5b43a" opacity="0.5" />

      <rect x="84" y="60" width="48" height="120" fill="#0a0a0f" />
      <rect x="92" y="68" width="8" height="8" fill="#f5b43a" />
      <rect x="108" y="68" width="8" height="8" fill="#f5b43a" />
      <rect x="92" y="84" width="8" height="8" fill="#ff3b3b" />
      <rect x="108" y="100" width="8" height="8" fill="#f5b43a" opacity="0.6" />

      <rect x="136" y="100" width="32" height="80" fill="#11111a" />
      <rect x="142" y="108" width="6" height="6" fill="#f5b43a" />

      <rect x="172" y="40" width="56" height="140" fill="#0a0a0f" />
      <rect x="180" y="48" width="8" height="8" fill="#f5b43a" />
      <rect x="196" y="48" width="8" height="8" fill="#f5b43a" />
      <rect x="212" y="64" width="8" height="8" fill="#ff3b3b" />
      <rect x="180" y="80" width="8" height="8" fill="#f5b43a" opacity="0.7" />
      <rect x="196" y="96" width="8" height="8" fill="#f5b43a" />

      <rect x="232" y="80" width="44" height="100" fill="#11111a" />
      <rect x="240" y="88" width="6" height="6" fill="#f5b43a" />
      <rect x="254" y="88" width="6" height="6" fill="#f5b43a" />

      <rect x="280" y="110" width="40" height="70" fill="#0a0a0f" />
      <rect x="288" y="118" width="6" height="6" fill="#f5b43a" />
    </svg>
  );
}
...
function PixelCharacter() {
  return (
    <svg viewBox="0 0 32 40" width={56} height={70} aria-hidden="true">
      ...
      <rect x="11" y="2" width="10" height="4" fill="#1a1924" />
      <rect x="13" y="8" width="2" height="2" fill="#0a0a0f" />
      <rect x="17" y="8" width="2" height="2" fill="#0a0a0f" />
      <rect x="14" y="12" width="4" height="1" fill="#ff3b3b" />
      <rect x="9" y="14" width="14" height="14" fill="#ff3b3b" />
      <rect x="9" y="14" width="14" height="2" fill="#f5b43a" />
      <rect x="9" y="28" width="6" height="10" fill="#1a1924" />
      <rect x="17" y="28" width="6" height="10" fill="#1a1924" />
...
function GameUI({ activeBeat }: { activeBeat: number }) {
  const dialogs = [
    {
      who: "Inner monologue",
      text: "First morning. Bed creaks. Coffee tin is half-empty. Rent in 9.",
    },
    {
      who: "Bumi · corner shop",
      text: "Tea's getting expensive. I keep paying for things I forget I bought. Can you look?",
    },
    {
      who: "You",
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-[var(--color-crimson)]">
        <span className="h-1 w-1 rounded-full bg-[var(--color-crimson)]" />
        {d?.who}
      </div>
